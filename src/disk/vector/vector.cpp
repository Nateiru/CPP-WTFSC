...
T DiskVector<T>::get(size_t pos) {
  block_id_t block_id = pos * sizeof(T) / BLOCK_SIZE;
  size_t offset = pos * sizeof(T) % BLOCK_SIZE;
...
void DiskVector<T>::put(size_t pos, T value) {
  ...
  *reinterpret_cast<T *>(buffer + offset) = value;
  io_.seekp(pos * sizeof(T));
  io_.write(buffer + offset, sizeof(T));
  io_.flush();
}
...
int main() {
  ...
  DiskVector<int> disk_vec("diskvector.txt");
